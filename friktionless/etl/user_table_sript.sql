...
create temp table user_daily_deposits (user_address string, as_of_date date, deposit_initiated_amt float64) as 
    select user_address, date(deposit_initiated_ts) as as_of_date, sum(deposit_initiated_amt) from transactions.fact_deposits group by 1, 2;

create temp table user_daily_withdrawals (user_address string, as_of_date date, withdrawal_initiated_amt float64) as 
    select user_address, date(withdrawal_initiated_ts) as as_of_date, sum(withdrawal_initiated_amt) from transactions.fact_withdrawals group by 1, 2;
    ...
    from (
        select 
            user.user_address, 
            user.as_of_date, 
            sum(dep.deposit_initiated_amt) OVER (order by user.as_of_date ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) as total_deposits_usd, 
        ...
        left join `friktion-dev.users.fact_user_firsts` user_firsts on user.user_address = user_firsts.user_address
    ) cd 
    left join (
        select user.user_address, user.as_of_date, coalesce(sum(dep.withdrawal_initiated_amt) OVER (order by user.as_of_date ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW),0.0) as total_withdrawals_usd
    ...
    left join (
        select 
            ...
            sum(fw.withdrawal_initiated_amt) as last_withdrawal_amt 
        from (
    ...
    left join (
        ...
        sum(ld_other.deposit_initiated_amt) as last_deposit_amt, 
        ...
        left join (
            select user_address, deposit_initiated_amt, deposit_initiated_epoch, deposit_initiated_ts, deposited_asset 
    ...
    left join (    
        select user.user_address, user.as_of_date, sum(dly_dep.deposit_initiated_amt) over (order by user.as_of_date rows between 30 preceding and current row) as tvl_delta_30_day 
        from individual_user user 
    ...
    left join (    
        select user.user_address, user.as_of_date, sum(dly_dep.deposit_initiated_amt) over (order by user.as_of_date rows between 60 preceding and current row) as tvl_delta_60_day 
        from individual_user user 
        left join user_daily_deposits dly_dep 
            on dly_dep.user_address = user.user_address
            and dly_dep.as_of_date = user.as_of_date
    ...
    left join (    
        select user.user_address, user.as_of_date, sum(dly_dep.deposit_initiated_amt) over (order by user.as_of_date rows between 90 preceding and current row) as tvl_delta_90_day 
        from individual_user user 
        left join user_daily_deposits dly_dep 
            on dly_dep.user_address = user.user_address
            and dly_dep.as_of_date = user.as_of_date
